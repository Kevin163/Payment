...
namespace GemstarPaymentCore.Business.BusinessHandlers.LcswPay
{
    ...
    /// 利楚商务扫呗预授权撤销接口
    /// </summary>
    public class LcswPayPreAuthCancelHandler : BusinessHandlerBase
    {
        ...
        public LcswPayPreAuthCancelHandler(ILogger<LcswPayBarcodePayHandler> log, ILcswPayClient client, IOptionsSnapshot<LcswPayOption> options, IOptionsSnapshot<BusinessOption> businessOption)
        {
            _log = log;
            _client = client;
        ...
        protected override string contentFormat => "merchantNo|terminalId|accessToken|terminalTrace|terminalTime|payTrace|payTime|outTradeNo|orderBody|attach";
        ...
        protected override async Task<HandleResult> DoHandleBusinessContentAsync(string[] infos)
        {
            try
            {
                ...
                var attach = infos[i++];
                //调用预授权撤销接口
                var request = new LcswPayPreAuthCancelRequest
                {
