    ...
    public class ChinaumsPayQueryHandler : BusinessHandlerBase
    {
        ...
        protected override async Task<HandleResult> DoHandleBusinessContentAsync(string[] infos)
        {
            try
            {
                ...
                {
                    return HandleResult.Success($"{response.BillStatus}|{response.BillPayment.Status}|{response.BillPayment.BuyerPayAmount}|{response.RefundBillPayment?.Status}|{response.RefundBillPayment?.RefundInvoiceAmount}");
                }
