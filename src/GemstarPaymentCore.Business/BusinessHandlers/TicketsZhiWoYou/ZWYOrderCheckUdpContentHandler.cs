...
namespace GemstarPaymentCore.Business.BusinessHandlers.TicketsZhiWoYou
{
    ...
    public class ZWYOrderCheckUdpContentHandler : BusinessHandlerBase
    {
        ...
        public ZWYOrderCheckUdpContentHandler(ILogger<ZWYOrderCheckUdpContentHandler> logger, IHttpClientFactory httpClientFactory, IOptionsSnapshot<BusinessOption> businessOption)
        {
            _log = logger;
        ...
        protected override async Task<HandleResult> DoHandleBusinessContentAsync(string[] contentInfo)
        {
            try
            {
                ...
                _log.LogInformation("ZWYOrderCheckUdpContentHandler", string.Format("发送给自我游的订单明细检票字符串：\r\n" + sendStr));
                string result;
#if MOCK
...
#endif
                _log.LogInformation("ZWYOrderCheckUdpContentHandler", string.Format("接收到自我游的订单明细检票结果：\r\n" + result));
