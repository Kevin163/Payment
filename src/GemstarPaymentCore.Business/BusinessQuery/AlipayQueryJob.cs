                    ...
                    if (records.Count > 0)
                    {
                        foreach (var record in records)
                        {
                            try
                            {
                                ...
                                log.LogInformation("AlipayQueryJob", "开始查询支付状态：" + record.ID);
                                ...
                                {
                                    //通信失败，直接记录失败原因到日志里面
                                    log.LogError("AlipayQueryJob", $"查询失败：{(response == null ? "没有收到返回内容":"状态："+response.Code+response.Msg)}");
                                }
                            } catch (Exception ex)
                            {
                                log.LogError("AlipayQueryJob", ex);
                            } finally
                            {
                            }
                        }
